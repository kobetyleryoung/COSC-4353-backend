...
class ProfileManagementService:
    ...
    def create_profile(
        ...
        with self._uow_manager.get_uow() as uow:
            ...
            existing = uow.profiles.get(user_id)
            if existing:
            ...
            if not existing_user:
                ...
                # In production, this should come from Auth0 or authentication system
                user = User(
                    id=user_id,
                    email=f"user_{user_id.value}@temp.com",
                    roles=set(),
                    auth0_sub=None
                )
                uow.users.add(user)
    ...
    def get_profile_by_user_id(self, user_id: UserId) -> Optional[Profile]:
        ...
        with self._uow_manager.get_uow() as uow:
            return uow.profiles.get(user_id)
    
    def get_all_profiles(self) -> List[Profile]:
        """Retrieve all profiles."""
        with self._uow_manager.get_uow() as uow:
            return uow.profiles.get_all()
